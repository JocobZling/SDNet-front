import React, {useState} from 'react';
import {Button, Row, Steps, Col, Popover, Image, Input, Progress} from 'antd';
import {
    SearchOutlined,
    DownloadOutlined,
    UserOutlined,
    SolutionOutlined,
    SmileOutlined
} from '@ant-design/icons';
import EdgeServer from '../../images/edgeServerOne1.png';
import Reaction from '../../images/reaction1.png';


const {Step} = Steps;

const {TextArea} = Input;

const Detection = ({pictureOnePosition, pictureTwoPosition, detectionId, beginDetection, getDetectionDetail, result, textAreaValue}) => {
    const [state, setState] = useState({
        current: 1
    })

    const begin = () => {
        beginDetection(detectionId);
        getDetectionDetail(detectionId);
    }
    const listItems = textAreaValue.map((item) =>
        <li>
            {item}
        </li>
    );
    const {current} = state;
    return (
        <div>
            {detectionId === 0 ? <Button icon={<SearchOutlined/>} onClick={begin} disabled>开始检测</Button> :
                <Button icon={<SearchOutlined/>} onClick={begin}>开始检测</Button>}
            {result.length === 2 ? <Button icon={<DownloadOutlined/>} style={{marginLeft: "20px"}}>
                下载交互详情
            </Button> : <Button icon={<DownloadOutlined/>} disabled style={{marginLeft: "20px"}}>下载交互详情</Button>}
            <Row align="middle" style={{marginTop: "20px"}}>
                <Col span={24}>
                    <Steps current={current}>
                        <Step title="随机数生成" icon={<UserOutlined/>} description="离线阶段生成所需的所有随机数"/>
                        <Step title="检测中" icon={<SolutionOutlined/>} description="双方服务器不断交互"/>
                        <Step title="完成检测" icon={<SmileOutlined/>} description="检测完成"/>
                    </Steps>
                </Col>
                <Col span={3} style={{marginTop: "20px"}}>
                    <Image
                        height={100}
                        src={pictureOnePosition}
                        preview={false}
                        fallback="data:image/png;base64,"
                    />
                </Col>
                <Col span={18}>
                    <Image src={EdgeServer} preview={false} width='100%' height={100}/>
                </Col>
                <Col span={3}>
                    结果：{result[0]}
                </Col>
                <Col span={6} style={{maxHeight: '100px', overflow: 'auto'}}>
                    {listItems}
                </Col>
                <Col span={17} offset={1}>
                    <Image src={Reaction} preview={false} height={100}/>
                </Col>
                <Col span={3}>
                    <Image
                        height={100}
                        src={pictureTwoPosition}
                        preview={false}
                        fallback="data:image/png;base64,"
                    />
                </Col>
                <Col span={18}>
                    <Image src={EdgeServer} preview={false} height={100}/>
                </Col>
                <Col span={3}>
                    结果：{result[1]}
                </Col>
                {/*<Col span={2} style={{marginTop: '20px'}}>*/}
                {/*    检测进度：*/}
                {/*</Col>*/}
                {/*<Col span={22} style={{marginTop: '20px'}}>*/}
                {/*    <Progress percent={textAreaValue.length} status="active"/>*/}
                {/*</Col>*/}
            </Row>
            {/*<Row style={{marginTop: "20px"}} gutter={[24, 20]}>*/}
            {/*    <Col span={24}>交互详情：*/}

            {/*    </Col>*/}
            {/*    /!*<Col span={24}>*!/*/}
            {/*    /!*    /!*<TextArea disabled maxLength={100} style={{height: 120, overflow: "visible",backgroundColor:'black',color:'white'}}*!/*!/*/}
            {/*    /!*    /!*          value={textAreaValue}/>*!/*!/*/}
            {/*    /!*    <ul>{listItems}</ul>*!/*/}
            {/*    /!*</Col>*!/*/}
            {/*</Row>*/}

        </div>
    )
}

export default Detection
